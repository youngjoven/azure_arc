...
sudo -u $adminUsername az connectedk8s connect --name $capiArcDataClusterName --resource-group $AZURE_RESOURCE_GROUP --location $location --tags 'Project=jumpstart_arcbox' --kube-config $CLUSTER_NAME.kubeconfig --kube-context "$CLUSTER_NAME-admin@$CLUSTER_NAME"
